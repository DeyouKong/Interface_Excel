...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTest(self, test) -> None:
        ...
        :return:
        """
        self.end_time = '{0:.2f}s'.format((time.time() - self.start_time))    # 修改精确至2位小数，屏蔽科学技术法
    ...
    def addFailure(self, test, err):
        ...
        output = self.complete_output()
        logs.append(output)
        logs.extend([self.error_or_failure_text(err)[-1]])    # 修改 失败用例的 日志显示。只展示错误信息的最后一行（断言信息）和 print打印信息
        self.failure_count += 1
    ...
    def add_test_img(*pargs):
        ...
        def _wrap(func):
            ...
            def __wrap(*args, **kwargs):
                ...
                # img_path = os.path.abspath('{}'.format(HTMLReport.img_path))    # 屏蔽自动获取存放截图的路径
                img_path = getattr(args[0], 'report_img_path')   # 从用例类属性获取 截图路径
                try:
